...
#if UNITY_EDITOR
using UnityEditor;
[CustomEditor(typeof(ColorIsolationDemo))]
public class ColorIsolationWarning : Editor
{
...
/// </remarks>
public class ColorIsolationDemo : MonoBehaviour
{
  ...
  private void Awake()
  {
    if (ColorIsolation.IsInRenderFeatures() == false)
    {
    ...
    this.enabled = ColorIsolation.IsInRenderFeatures();
  }

  private void Start()
  {
    settings = ColorIsolation.GetSettings();
    ResetEffect();
  }
  ...
  private void OnGUI()
  {
    ...
    GUILayout.BeginHorizontal();
    {
      GUILayout.BeginVertical("box", GUILayout.Width(400.0f), GUILayout.Height(Screen.height));
      {
        ...
        GUILayout.Label("COLOR ISOLATION DEMO", styleTitle);
